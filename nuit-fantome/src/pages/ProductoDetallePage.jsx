...
const loadAdminArr = () => { try {return JSON.parse(localStorage.getItem(ADMIN_PRODUCTS_KEY)||"[]");} catch {return [];} };
const saveAdminArr = (arr) => {
...
/* ====== Normalización admin → tienda ====== */
// deduce nombre de categoría por idCategoria usando el catálogo base
const uniqueCats = Array.from(new Set(base.map(p=>p.categoria)));
const CAT_ID_TO_NAME = {}; uniqueCats.forEach((n,i)=> CAT_ID_TO_NAME[i+1]=n);

function normalizeFromAdmin(ap){
  let categoria =
    ap.categoria ||
    CAT_ID_TO_NAME[ap.idCategoria] ||
    (ap.idCategoria ? `Categoría #${ap.idCategoria}` : (base.find(b=>b.id===ap.id)?.categoria || "Otros"));

  return {
    id: ap.id,
    nombre: ap.nombre,
    precio: Number(ap.precio || 0),
    img: ap.img || (base.find(b=>b.id===ap.id)?.img) || "/assets/img/placeholder.jpg",
    desc: ap.desc || (base.find(b=>b.id===ap.id)?.desc) || "",
    categoria,
    stock: typeof ap.stock === "number" ? ap.stock : (base.find(b=>b.id===ap.id)?.stock ?? undefined),
    opciones: Array.isArray(ap.opciones) ? ap.opciones : (base.find(b=>b.id===ap.id)?.opciones || [])
  };
}

function buildMergedCatalog(){
  const map = new Map(base.map(p=>[p.id,p]));
  loadAdminArr().forEach(ap=>{
...
/* ====== Stock unificado (lee/escribe en nf_productos) ====== */
...
function persistStockChange(id, optName, delta){
  const arr = loadAdminArr();
  let p = arr.find(x=>x.id===id);
  if(!p){
    const b = base.find(z=>z.id===id) || { id };
    p = { ...b, id };
    arr.push(p);
  }
  if(optName){
    if(!Array.isArray(p.opciones)) p.opciones = [];
    let oo = p.opciones.find(o=>o.t===optName);
    if(!oo){ oo = {t: optName, s: 0}; p.opciones.push(oo); }
...
export default function ProductoDetallePage(){
  ...
  const [catalogo, setCatalogo] = useState(()=>buildMergedCatalog());
  useEffect(()=>{
    const reload = () => setCatalogo(buildMergedCatalog());
    window.addEventListener("nf:productos:update", reload);
    window.addEventListener("storage", (e)=>{ if(e.key===ADMIN_PRODUCTS_KEY) reload(); });
    return () => {
  ...
  const disponibles = getUnifiedStockFor(prod.id, opt || "");
  const agotado = disponibles !== null && disponibles <= 0;

  const recents = getRecents()
    .filter(x=>x!==prod.id)
    .map(rid=>catalogo.find(p=>p.id===rid))
    .filter(Boolean)
    .slice(0,8);
  ...
  function addToCart(){
    if(agotado){ alert("Sin stock disponible para esta opción."); return; }
    let n = Math.max(1, qty|0);
    if(disponibles !== null && n > disponibles){
      alert(`Solo quedan ${disponibles} unidades disponibles. Ajustamos tu cantidad.`);
      n = disponibles;
    }
    const cart = loadCart();
    const kopt = opt || "";
    const it = cart.find(i=>i.id===prod.id && (i.opt||"")===kopt);
    if(it) it.qty += n; else cart.push({id:prod.id, qty:n, opt:kopt});
    saveCart(cart);
    if(disponibles !== null) persistStockChange(prod.id, kopt || null, -n);
    ...
    <>
      <main style={{width:"min(1100px,95%)", margin:"24px auto 32px", color:"#fff"}}>
        ...
        <section style={{display:"grid", gridTemplateColumns:"1fr 1fr", gap:24}}>
          <img
            src={prod.img || "/assets/img/placeholder.jpg"}
            alt={prod.nombre}
          ...
          <div>
            ...
            {disponibles !== null && (
              <div style={{marginTop:8, color:"#fff"}}>Disponible: {disponibles}</div>
            )}
            ...
            <div style={{display:"flex", gap:8, marginTop:12}}>
              <input
                ...
                min={1}
                value={qty}
                onChange={e=>setQty(Math.max(1, e.target.value|0))}
